    ...
    if(kernel->getAttr<CUDAGlobalAttr>()) {
        if(isCalledFromKernel_) {
            ...
            if(kernel->doesThisDeclarationHaveABody()) {
                ...
                SourceLocation beginBody = cast<CompoundStmt>(kernel->getBody())->body_front()->getBeginLoc();
                rewriter_.InsertTextBefore(beginBody, ssPrologue.str());

                // Close guard
                std::stringstream ssEpilogue;
                ssEpilogue << ";\n}\n";
                SourceLocation endBody = cast<CompoundStmt>(kernel->getBody())->body_back()->getEndLoc();
        ...
        if(sm.isInMainFile(sm.getExpansionLoc(kernel->getBeginLoc()))) {
            rewriter_.RemoveText(SourceRange(kernel->getBeginLoc(), kernel->getEndLoc()));
        }
    }
