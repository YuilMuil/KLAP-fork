...
bool DivergenceEliminationTransform::VisitStmt(Stmt *s) {
    ...
    if(CUDAKernelCallExpr* kernelCall = dyn_cast<CUDAKernelCallExpr>(s)) {
        ...
        if(analyzer_.isCallFromKernelCandidate(kernelCall)) {
            ...
            // Rewrite
            rewriter_.InsertTextBefore(outermostDivergentStmt->getBeginLoc(), ssBefore.str());
            rewriter_.ReplaceText(SourceRange(kernelCall->getBeginLoc(), kernelCall->getEndLoc()), ssReplace.str());
            rewriter_.InsertTextAfterToken(outermostDivergentStmt->getEndLoc(), ssAfter.str());
        ...
        if(analyzer_.isAPIFromKernelCandidate(CE)) {
            if(FunctionDecl* f = CE->getDirectCallee()) {
                if(f->getDeclName().getAsString() == "cudaMalloc") {
                    ...
                    // Rewrite
                    rewriter_.InsertTextBefore(outermostDivergentStmt->getBeginLoc(), ssBefore.str());
                    rewriter_.ReplaceText(SourceRange(CE->getBeginLoc(), CE->getEndLoc()), ssReplace.str());
                    rewriter_.InsertTextAfterToken(outermostDivergentStmt->getEndLoc(), ssAfter.str());
