...
BENCHMARK=MST
NAME=mst
DATASETS=("-g ../../datasets/kron_g500-simple-logn16.gr" "-g ../../datasets/cnr-2000.gr")
RUNS=1
FILE=$NAME.csv
    ...
    for version in $VERSIONS; do
        if [ -f $NAME.$version ]; then
            for r in `seq 1 $RUNS`; do
                  TIMES=`./$NAME.$version $dataset -o 0`
                  FTIME=`echo $TIMES | cut -d , -f 1`
                  VTIME=`echo $TIMES | cut -d , -f 2`
                  echo ${NAME}f, $dataset, $version, N/A, N/A, N/A, $r, $FTIME | tee -a $FILE
                  echo ${NAME}v, $dataset, $version, N/A, N/A, N/A, $r, $VTIME | tee -a $FILE
            done
        fi
    done
done
      ...
      for version in $THRESHOLD_VERSIONS; do
          if [ -f $NAME.$version ]; then
              for r in `seq 1 $RUNS`; do
                  ...
                  FTIME=`echo $TIMES | cut -d , -f 1`
                  VTIME=`echo $TIMES | cut -d , -f 2`
                  echo ${NAME}f, $dataset, $version, $threshold, N/A, N/A, $r, $FTIME | tee -a $FILE
                  echo ${NAME}v, $dataset, $version, $threshold, N/A, N/A, $r, $VTIME | tee -a $FILE
              done
          fi
      done
  done
      ...
      for version in $COARSE_VERSIONS; do
          if [ -f $NAME.$version ]; then
              for r in `seq 1 $RUNS`; do
                  ...
                  FTIME=`echo $TIMES | cut -d , -f 1`
                  VTIME=`echo $TIMES | cut -d , -f 2`
                  echo ${NAME}f, $dataset, $version, N/A, $coarse_factor, N/A, $r, $FTIME | tee -a $FILE
                  echo ${NAME}v, $dataset, $version, N/A, $coarse_factor, N/A, $r, $VTIME | tee -a $FILE
              done
          fi
      done
  done
        ...
        for version in $THRESHOLD_COARSE_VERSIONS; do
            if [ -f $NAME.$version ]; then
                for r in `seq 1 $RUNS`; do
                  ...
                  FTIME=`echo $TIMES | cut -d , -f 1`
                  VTIME=`echo $TIMES | cut -d , -f 2`
                  echo ${NAME}f, $dataset, $version, $threshold, $coarse_factor, N/A, $r, $FTIME | tee -a $FILE
                  echo ${NAME}v, $dataset, $version, $threshold, $coarse_factor, N/A, $r, $VTIME | tee -a $FILE
                done
            fi
        done
    done
        ...
        for version in $DYNAMIC_AGG_VERSIONS_ALL; do
          if [ -f $NAME.$version ]; then
            for r in $(seq 1 $RUNS); do
             ...
             FTIME=`echo $TIMES | cut -d , -f 1`
             VTIME=`echo $TIMES | cut -d , -f 2`
             echo ${NAME}f, $dataset, $version, $threshold, $coarse_factor, $aggregation_gran, $r, $FTIME | tee -a $FILE
             echo ${NAME}v, $dataset, $version, $threshold, $coarse_factor, $aggregation_gran, $r, $VTIME | tee -a $FILE
            done
          fi
        done
      done
