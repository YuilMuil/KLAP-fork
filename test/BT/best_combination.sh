...
update_dataset() {
  tesselation=$1
  curvature=$2
  sed -i -e "s/MAX_TESSELLATION .*/MAX_TESSELLATION ${tesselation}/g" $NAME.csv
  sed -i -e "s/CURVATURE .*/CURVATURE ${curvature}/g" $NAME.csv
}
...
run_benchmark() {
  ...
  cdp_average=$(./${NAME}.cdp -n "${dataset_path}" -o0)
  no_cdp_average=$(./${NAME}.nocdp -n "${dataset_path}" -o0)
  aggregation_average=$(./${NAME}.${aggregation_version} -n "${dataset_path}" -o0)
  threshold_average=$(./${NAME}.tcdp -n "${dataset_path}" -o0)
  coarsening_average=$(./${NAME}.ccdp1 -n "${dataset_path}" -o0)
  ...
  cdp_t_c_average=$(./${NAME}.ctcdp1 -n "${dataset_path}" -o0)
  ...
  cdp_t_a_average=$(./${NAME}.${cdp_t_a_aggregation_version} -n "${dataset_path}" -o0)
  ...
  cdp_c_a_average=$(./${NAME}.${cdp_c_a_aggregation_version} -n "${dataset_path}" -o0)
  ...
  cdp_t_c_a_average=$(./${NAME}.${cdp_t_c_a_aggregation_version} -n "${dataset_path}" -o0)
...
update_dataset 32 16
run_benchmark "T0032C16" "20000" "ag" 32 8 32 16 "tag" 32 "cag1" 4 "ctag1" 32 256

update_dataset 2048 64
run_benchmark "T2048C64" "20000" "ag" 256 1024 1024 256 "tab" 2048 "cag1" 1024 "ctab1" 1024 2
